    ...
    public int jump() {
        ...
        for (int j = 0; j < Lesson_8.walls.size(); j++) {
            if (this.getheightToJump() >= Lesson_8.walls.get(j).getheightOfWall()) {
                System.out.println("Робот " + this.getName() + " перепрыгнул через " + Lesson_8.walls.get(j).getName() + " высотой " + Lesson_8.walls.get(j).getheightOfWall() + " м и побежал дальше");
                successJumps++;
            } else {
                System.out.println("Робот " + this.getName() + " не смог перепрыгнуть через " + Lesson_8.walls.get(j).getName() + " высотой " + Lesson_8.walls.get(j).getheightOfWall() + " м.");
                break;
            }
        }
        return successJumps;
