    ...
    public int jump() {
        ...
        for (int j = 0; j < Lesson8.walls.size(); j++) {
            if (this.getheightToJump() >= Lesson8.walls.get(j).getheightOfWall()) {
                System.out.println("Человек " + this.getName() + " перепрыгнул через " + Lesson8.walls.get(j).getName() + " высотой " + Lesson8.walls.get(j).getheightOfWall() + " м и побежал дальше");
                successJumps++;
            } else {
                System.out.println("Человек " + this.getName() + " не смог перепрыгнуть через " + Lesson8.walls.get(j).getName() + " высотой " + Lesson8.walls.get(j).getheightOfWall() + " м.");
                break;
            }
        }
        return successJumps;
